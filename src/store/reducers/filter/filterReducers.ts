...
export function filterReducers(state: IFilterState = initialState, action: FilterActionTypes): IFilterState {
  ...
  switch (action.type) {
    ...
    case ADD_FILTER_BY_ITEM_TYPE:
      return {
        ...state,
        itemType: [...state.itemType, action.payload],
      };
    case REMOVE_FILTER_BY_ITEM_TYPE:
      return {
        ...state,
        itemType: state.itemType.filter((filter) => filter !== action.payload),
      };
    case SET_ORDER_BY:
